...
public class EncoderTests {
    ...
    public void testEncodeDoubleArray() throws IOException {
        ...
        Assert.assertEquals("09000300000000000000002059400000000000006940000000000088B340", ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    ...
    public void testEncodeLongArray() throws IOException {
        ...
        Assert.assertEquals("0700040000006400000000000000C800000000000000404B4C000000000018FCFFFFFFFFFFFF", ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    }

    @Test
    public void testTimestampArray() throws IOException {
        MutableDirectBuffer buffer = new ExpandableArrayBuffer(1024);
        int offset = 0;
        /*
        -8!(`timestamp$1990.01.02;`timestamp$2021.02.04; `timestamp$2019.12.16)
         */
        long[] vector = {TimeUtils.toKdbTimestamp("1990-01-02T00:00:00.00Z"), TimeUtils.toKdbTimestamp("2021-02-04T00:00:00.00Z"),
                TimeUtils.toKdbTimestamp("2019-12-16T00:00:00.00Z")};
        int size = KdbEncoder.encodeNanosToTimestamp(buffer, 0, vector, ByteOrder.LITTLE_ENDIAN);
        assertEquals(30, size);
        Assert.assertEquals("0c0003000000000053a62b4f9ffb0000b77289153d09000057521d64bd08".toUpperCase(), ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    ...
    public void testEncodeCharArray() throws IOException {
        ...
        assertEquals(9, size);
        Assert.assertEquals("0A000300000063617A", ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    ...
    public void testEncodeTimespanVector() {
        ...
        assertEquals(30, size);
        Assert.assertEquals("10000300000001000000000000000200000000000000083129C4B28F1A09", ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    }
    ...
    public void testEncodeRpcCall() {
        ...
        String bytesToHex = ByteUtils.bytesToHex(buffer.byteArray(), 0, size);
    ...
    public void testEncodeList() {
        ...
        Assert.assertEquals("000003000000F5752E75706400F571756F7465000700030000000100000000000000020000000000000040420F0000000000", ByteUtils.bytesToHex(buffer.byteArray(), 0, size));
    }
